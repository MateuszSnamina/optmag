...
{
    constrained_optimizer::OptimizationProblemDefinition& problem_definition = *reinterpret_cast<constrained_optimizer::OptimizationProblemDefinition*>(ptr);
    // Expects:
...
void optimize(OptimizationProblemDefinition problem_definition, OptimizationMethodParams params, bool optguard) {
    ...
    std::visit(SetupVisitor{state}, params);
    ...
    // Optimize:
    alglib::minnlcoptimize(state, nlcfunc_jac);
    ...
    std::cout << "ARGLIB RESULT| " << x1_arglib.tostring(2).c_str() << std::endl;
